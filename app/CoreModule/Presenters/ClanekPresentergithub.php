 ...
 */
class ClanekPresenterGit extends BasePresenter
{
    /** @var int */
	private $article_id;
	...
	public function nactiAutory()// : void 
	{
        ...
        if (!array_search($username, array_keys($allautors))){
    ...
    */
    public function renderDefault($url,$filter = null)
    {
        ...
        if ($filter == 1) {$this-> template->all = true; $approve=true;}
        else {$this->template->actual = $filter; $this->template->all = false; $approve=false;}

        //pouze schválené oblíbené články pro nepřihlášené 
        $this->template->favourites = /*$favourites =*/ $this->clanekManager->getClankyViews(!($this->getUser()->isLoggedIn())); //filtr schválené!
    ...
    public function handleAscendingClanky($url,$filter) {
        $filter ? $filter = '' : $filter = ' DESC';
        $this->redirect('Clanek:default', $url, $filter);
    }
    ...
    public function actionEditor(string $url = null, $adminnote = null, $novaVerze = null)
    {
        ...
        if ($url) {
            if (!($clanek = $this->clanekManager->getClanek($url))){
                ...
                $categories = $this->categoryManager->getAllCategoryName();
                $values['category'] = (array_search($clanek->category, $categories)); 
                $values['url'] = $url;
    ...
    protected function createComponentEditorForm()
    {
        ...
        if (($parameters['adminnote'] == TRUE)) {
            $form->addText('notes', 'Zapiš admin poznámku')->setRequired();//->setDefaultValue('Vložte poznámku')
        ...
        elseif($parameters['novaVerze'] == TRUE){
            ...
            //část formuláře pro editaci poznámky nené otevřena následující se pouze předávají dál
        ...
        $form->onSuccess[] = function (Form $form, ArrayHash $values) {
            ...
            try {
                ...
                if (isset($clanek['notes'])) {
                    ...
                    if ($values['notes']) $polenotes[] = ($values['notes']);
                ...
                //pokud jde o nový článek ulož usera jako autora!-editace na autor neuveden možné při dalčší editaci
